            ...
            (tags.value = store.state.tags
                .map((tag: CosmicTag) => {
                    return {
                        slug: tag.slug,
                        title: tag.title,
                        postSlugs: tag.metadata.posts.map(
                            (post: Post) => post.slug
                        ),
                    };
                })
                .filter(
                    (tag: Tag) =>
                        tag.postSlugs.indexOf(params.slug as string) !== -1
                ))
    );
} else {
    tags.value = store.state.tags
        .map((tag: CosmicTag) => {
            return {
                slug: tag.slug,
                title: tag.title,
                postSlugs: tag.metadata.posts.map((post: Post) => post.slug),
            };
        })
        .filter(
            (tag: Tag) => tag.postSlugs.indexOf(params.slug as string) !== -1
        );
}
</script>
